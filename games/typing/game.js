...
function renderStats() {
  ...
  const bars = graphRuns.length ? graphRuns.slice().reverse().map((run) => `<div class="bar-group"><div class="bar wpm-bar" style="height:${Math.max(8, run.wpm / maxWpm * 100)}%" title="${formatNumber(run.wpm)} WPM"></div><div class="bar accuracy-bar" style="height:${Math.max(8, run.accuracy)}%" title="${formatNumber(run.accuracy)}% accuracy"></div></div>`).join('') : '<p class="empty-state">Complete a run to grow your graph.</p>';
  ...
  typingView.innerHTML = `<div class="stats-summary"><div><span>Latest run</span><strong>${recent[0] ? `${formatNumber(recent[0].wpm)} WPM` : '—'}</strong></div><div><span>Personal best</span><strong>${best ? `${formatNumber(best.wpm)} WPM` : '—'}</strong></div><div><span>Rolling average</span><strong>${recent.length ? `${formatNumber(average)} WPM` : '—'}</strong></div><div><span>Trend</span><strong class="${trend >= 0 ? 'positive' : 'negative'}">${recent.length > 1 ? `${trend >= 0 ? '+' : ''}${formatNumber(trend)} WPM` : '—'}</strong></div></div><div class="chart-block"><div class="chart-key"><span><i class="key-wpm"></i>WPM</span><span><i class="key-accuracy"></i>accuracy</span></div><div class="bar-chart">${bars}</div></div><div class="runs-table"><div class="table-heading"><h3>Last 10 runs</h3><span>${history.length} total</span></div>${rows}</div>`;
}
